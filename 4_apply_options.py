...
from tkinter import *
# filedialog는 서브 모듈 이라서 __all__에 포함되어 있지 않음. 따라서 import해줘야 함
...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", filetypes=(
        ("PNG 파일", "*.png"), ("모든 파일", "*.*")), initialdir="C:/")  # 최초에 C:/ 경로를 보여줌

    # 사용자가 선택한 파일 목록
    for file in files:
        list_file.insert(END, file)

# 선택 삭제 # revers는 원래값에 영향o, reversed는 원래 값에 영향x
...
btn_del_file = Button(file_frame, padx=5, pady=5,
                      width=12, text="선택 삭제", command=del_file)
